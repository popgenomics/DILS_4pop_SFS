...
def get_frequencies(aln):
	...
	for i in range(len(aln_list[0])):  # Assuming all sequences are the same length
		sum_ones = sum(1.0*aln_list[j][i] for j in range(len(aln)))
		frequencies.append(sum_ones / (1.0*len(aln)))
	return frequencies
...
#					DtajD.append(tajD(tmpD['pi'], thetaD_locus, nSamD[nLoci_cnt - 1], tmp['sxD']+tmp['ss'], a1_spD[nLoci_cnt-1], a2_spD[nLoci_cnt-1]))
	...
	if nLoci_cnt != 0 and len(sxA) == nLoci:
		...
		# DISPLAY THE SFS 
		vectorized_sfs = [str(sfs[i][j][k][l]) for i in range(nBinFreq) for j in range(nBinFreq) for k in range(nBinFreq) for l in range(nBinFreq)]
